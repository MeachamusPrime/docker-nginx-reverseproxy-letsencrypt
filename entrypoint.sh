...
do
  ...
  if [ -z "${!proxy}" ]; then
    ...
    DOMAIN=${!host} envsubst '$DOMAIN' < /tmp/multiservice.conf.header > "/conf/${FILE_NAME}"
    ...
    do
      ...
      LOCATION=${!location} PROXY=${!total_proxy} envsubst '$PROXY,$LOCATION' < /tmp/multiservice.conf.template >> "/conf/${FILE_NAME}"
    done
    DOMAIN=${!host} envsubst '$DOMAIN' < /tmp/multiservice.conf.footer >> "/conf/${FILE_NAME}"
    continue;
  fi
  FILE_NAME=$(echo $service | tr '[:upper:]' '[:lower:]').conf
  echo "Generating nginx configuration file \"${FILE_NAME}\" for \"${!host}\"."
